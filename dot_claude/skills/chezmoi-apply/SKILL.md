...
ソースを編集する。ターゲットを直接編集しない。

このリポジトリは chezmoi のソースディレクトリそのもの。編集対象は必ずソースファイル（リポジトリ内のファイル）であり、`$HOME` 配下のライブファイルではない。ライブファイルを直接編集しても、次の `chezmoi apply` で上書きされて失われる。
...
- `modify_` スクリプト: 出力されたファイルではなくスクリプト本体を編集する（自動追記される設定はこちらで管理する）。
...
空であれば成功。ソース編集、diff 確認、apply、反映確認まで完了したことを報告する。再起動やリロードが必要なツール（WezTerm 等）は次回起動時に反映されることも伝える。
