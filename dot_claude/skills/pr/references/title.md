...
簡潔第一。`<prefix>:` を除く本体は主目的だけを1フレーズで言い切り、20字前後を狙う（上限30字）。変更の列挙・手段・補足はタイトルに入れず本文へ回す。「〜し、〜する」のような複数事項の接続や `（…）` での補足は避け、文末の「〜する」等も削る。

## 字数を削る（必須）

規約に適合させた後、本体を「読み手に伝わる最小の字数」まで削ってから emit する。

1. 残す要素は「どこで」「何が」の2つだけ。手段・原因・条件の詳細は本文へ。
2. 落とす語：「〜の不具合」「〜の問題」「〜した際に」「〜エラー」「〜機能」など、prefix や文脈から自明な語。
3. 助詞と「の」の連鎖を減らし、名詞止めにする。
4. 削って意味が通らなくなった語だけ戻す。

```text
fix: 管理画面でユーザー削除ボタンを連打した際に2件目以降が500エラーになる不具合を修正   # 長い
fix: ユーザー削除の連打で500になる不具合を修正                                       # まだ削れる
fix: ユーザー削除連打時の500を修正                                                   # これで伝わる
```
