...
このファイル自体は skill 実行者向けの説明。実際に出力する MD ファイルの中身は、下の ```yaml ``` フェンス内だけであり、この見出しや本文の解説は出力に含めない。

デスクトップ（`~/Desktop`）に `coderabbit-review-<PR番号>.md` として出力する。指摘 0 件でも生成し `findings: []` とする。出力 MD の中身は単一の YAML fenced code block のみ（見出し・タイトル行・説明文は無し）。人間ではなくエージェントが再読する前提なので、冗長な散文は書かず構造化フィールドで埋める。
